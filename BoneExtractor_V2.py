...
class BoneExtractorV2:
    """V2元宇宙专版 (分层策略 + 单阈值硬骨 + Taubin平滑)"""
    
    def __init__(self, config=None):
        self.AI_AVAILABLE = True
        self.config = {
            "use_high_resolution": False, "ai_task": "total",
            # 分层参数
    ...
    def extract_bone(self, ct_data, spacing):
        ...
        try:
            ...
            return self._extract_bone_logic(ct_data, spacing, ai_output_path)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
    
    def _extract_bone_logic(self, ct_data, spacing, ai_output_path):
        import nibabel as nib
        ai_data = nib.load(ai_output_path).get_fdata()
        
        # ========== 分层策略（注意 V2 保留骨髓瘤靶向区域，但仍使用分层捕获额外骨骼） ==========
        V2_MYELOMA_LABELS = list(range(25, 51)) + list(range(69, 79)) + [91] + list(range(92, 117))
        precise_bone = np.isin(ai_data, V2_MYELOMA_LABELS)
        ...
        # ========== 原有单阈值截断，作用域改为 combined_mask ==========
        bone_mask = (ct_smooth > self.config["bone_threshold"]) & combined_mask
    ...
    def create_mesh(self, bone_mask, spacing, label_data=None, pet_data=None):
        ...
        bone_mesh = bone_mesh.smooth_taubin(
            n_iter=self.config["taubin_iter"],
            pass_band=self.config["taubin_pass_band"],
        ...
        if label_data is not None and np.any(label_data):
            ...
            tumor_mesh = tumor_mesh.smooth(n_iter=30, relaxation_factor=0.02)
    ...
    def export_models(self, meshes, output_dir, case_name):
        ...
        for mesh_name, mesh in meshes.items():
            mesh.save(os.path.join(output_dir, f"{case_name}_V2_{mesh_name}.obj"))